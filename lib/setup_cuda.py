...
PATH = '%s;%s'%(PATH, r'D:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\BIN\x86_amd64')
...
cythonize('model/nms/gpu_nms.pyx',language='c++')
...
cudamat_ext = Extension('model.nms.gpu_nms',
                        sources=[
                                'model/nms/gpu_nms.cu', 
                                ],
                        language='c++',
                        libraries=cuda_libs,
...
class CUDA_build_ext(build_ext):
    ...
    def spawn(self, cmd, search_path=1, verbose=0, dry_run=0):
        ...
        """
        cpp = [(i,e) for i, e in enumerate(cmd) if e.endswith('.cpp') ]
        for i,e in cpp:
            src_cpp = e[3:] 
            assert os.path.exists(src_cpp)
            return
        
        #if (sys.platform == 'win32' and cmd[0] == '' ):
        #    cmd = [ e for e in cmd if not e.endswith('gpu_nms.obj') ]            
        ...
        if (sys.platform == 'darwin' and len(cmd) >= 2 and cmd[0] == 'nvcc' and
            ...
            for idx, c in enumerate(cmd):
                ...
                elif c.startswith('/MANIFEST:EMBED'): del cmd[idx] 
